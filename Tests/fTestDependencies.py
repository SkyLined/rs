...
def fasSortedModuleNames(asList):
  # Ignore case and '_' as a prefix
  return sorted(asList, key = lambda sItem: sItem.lstrip("_").lower());
...
def fTestDependencies():
  ...
  asUnexpectedDependencyPythonInteralModuleBaseNames = [
    ...
    if (
      sModuleName not in asExpectedDependencyPythonInternalModuleBaseNames
    )
  ];
  ...
  if asUnexpectedDependencyPythonInteralModuleBaseNames or asSuperflousDependencyPythonInternalModuleBaseNames:
    for sModuleName in fasSortedModuleNames(asLoadedPythonInteralModuleBaseNames + asSuperflousDependencyPythonInternalModuleBaseNames):
      print "%s %s" % (
  ...
  if asUnexpectedDependencyModuleNames or asSuperfluousDependencyModuleNames:
    for sModuleName in fasSortedModuleNames(dsLoadedDependencyModules_by_sName.keys() + asSuperfluousDependencyModuleNames):
      print "%s %s%s" % (
  ...
  if asUnexpectedDependencyPythonInteralModuleBaseNames or asSuperflousDependencyPythonInternalModuleBaseNames \
    ...
    for sModuleName in fasSortedModuleNames(sys.modules.keys()):
